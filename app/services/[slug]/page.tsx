...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main>
    <SiteHeader/>
    <section className="hero"><div className="container"><Link href="/" className="back-link"><ArrowLeft size={16}/> Back home</Link><p className="eyebrow">{contractor.location.serviceAreaLabel}</p><h1>{service.name}</h1><p className="hero-copy">{service.shortDescription}</p><div className="actions"><a className="button" href={contractor.business.phoneHref}><Phone size={18}/> Call {contractor.business.phone}</a><Link className="button secondary" href="/#quote">Request service <ArrowRight size={18}/></Link></div></div></section>
    ...
    <section className="section tinted"><div className="container narrow"><p className="eyebrow">Need help now?</p><h2>Tell us what is happening.</h2><p>Use the request form on the homepage or call the business directly to confirm availability for your address.</p><Link className="button" href="/#quote">Request service <ArrowRight size={18}/></Link></div></section>
